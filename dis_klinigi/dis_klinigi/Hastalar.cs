...
namespace dis_klinigi
{
    public partial class Hastalar : Form
    {
        ...
        public void VerileriTazele()
        {
            using (SqlConnection con = DbConnection.GetConnection())
            {
                try
                {
                    ...
                    string query = "SELECT * FROM Hastalar_tbl WHERE HastaTCNumarasi LIKE @ara OR HastaAdi LIKE @ara OR HastaSoyadi LIKE @ara";
        ...
        {
            if (!string.IsNullOrWhiteSpace(txtadi.Text) && !string.IsNullOrWhiteSpace(txtsoyadi.Text) && !string.IsNullOrWhiteSpace(txttc.Text) && !string.IsNullOrWhiteSpace(mtxttelno.Text))
            {
                ...
                using (SqlConnection con = DbConnection.GetConnection())
                {
                    try
                    {
                        using (SqlCommand cmd = new SqlCommand("INSERT INTO Hastalar_tbl (HastaTCNumarasi, HastaAdi, HastaSoyadi, HastaCinsiyeti, HastaTelefonNumarasi) VALUES(@tc, @adi, @soyadi, @cinsiyet, @telno)", con))
                        {
        ...
        private void btndüzenle_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(txtadi.Text) && !string.IsNullOrWhiteSpace(txtsoyadi.Text) && !string.IsNullOrWhiteSpace(txttc.Text) && !string.IsNullOrWhiteSpace(mtxttelno.Text))
            {
                ...
                using (SqlConnection con = DbConnection.GetConnection())
                {
                    try
                    {
                        using (SqlCommand cmd = new SqlCommand("UPDATE Hastalar_tbl SET HastaTCNumarasi = @tc, HastaAdi = @adi, HastaSoyadi = @soyadi, HastaCinsiyeti = @cinsiyet, HastaTelefonNumarasi = @telno WHERE HastaID = @id", con))
                        {
        ...
        private void btnsil_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = DbConnection.GetConnection())
            {
                try
                {
                    using (SqlCommand cmd = new SqlCommand("DELETE FROM Hastalar_tbl WHERE HastaID = @id", con))
                    {
                        cmd.Parameters.AddWithValue("@id", ID);
                        con.Open();
                        cmd.ExecuteNonQuery();
        ...
        private void dataGridView1_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                ...
                txttc.Text = dataGridView1.Rows[e.RowIndex].Cells[1].Value?.ToString() ?? string.Empty;
                txtadi.Text = dataGridView1.Rows[e.RowIndex].Cells[2].Value?.ToString() ?? string.Empty;
                txtsoyadi.Text = dataGridView1.Rows[e.RowIndex].Cells[3].Value?.ToString() ?? string.Empty;
                mtxttelno.Text = dataGridView1.Rows[e.RowIndex].Cells[5].Value?.ToString() ?? string.Empty;
            }
