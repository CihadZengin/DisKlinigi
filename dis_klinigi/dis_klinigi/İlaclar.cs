...
namespace dis_klinigi
{
    public partial class İlaclar : Form
    {
        ...
        public void VerileriTazele()
        {
            using (SqlConnection con = DbConnection.GetConnection())
            {
                try
                {
                    ...
                    using (SqlDataAdapter adapt = new SqlDataAdapter("SELECT * FROM Recete_tbl", con))
                    {
                        adapt.Fill(dt);
        ...
        private void veriara()
        {
            using (SqlConnection con = DbConnection.GetConnection())
            {
                try
                {
                    ...
                    string query = "SELECT * FROM Recete_tbl WHERE İlacAdi LIKE @ara";

                    using (SqlDataAdapter adptara = new SqlDataAdapter(query, con))
                    {
                        adptara.SelectCommand.Parameters.AddWithValue("@ara", "%" + txtara.Text.Trim() + "%");
                        adptara.Fill(dtara);
        ...
        {
            if (string.IsNullOrWhiteSpace(txtadi.Text) || string.IsNullOrWhiteSpace(txtkullanimi.Text) || string.IsNullOrWhiteSpace(rtxtilacdetay.Text))
            {
            ...
            using (SqlConnection con = DbConnection.GetConnection())
            {
                try
                {
                    using (SqlCommand cmd = new SqlCommand("INSERT INTO Recete_tbl (İlacAdi, Kullanımkosullari, İlacDetaylari) VALUES (@ilacadi, @kullanim, @kosul)", con))
                    {
                        cmd.Parameters.AddWithValue("@ilacadi", txtadi.Text);
                        cmd.Parameters.AddWithValue("@kullanim", txtkullanimi.Text);
                        cmd.Parameters.AddWithValue("@kosul", rtxtilacdetay.Text);

                        con.Open();
                        cmd.ExecuteNonQuery();
                    }
                    VerileriTazele();
                    MessageBox.Show("İlaç Başarılı Bir Şekilde Kayıt Edildi");
                    temizle();
                }
                catch (Exception ex1)
                {
        ...
        private void btndüzenle_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = DbConnection.GetConnection())
            {
                try
                {
                    using (SqlCommand cmd = new SqlCommand("UPDATE Recete_tbl SET İlacAdi=@ilacadi, KullanımKosullari=@kullanim, İlacDetaylari=@kosul WHERE İlacID=@ID", con))
                    {
        ...
        private void btnsil_Click(object sender, EventArgs e)
        {
            ...
            using (SqlConnection con = DbConnection.GetConnection())
            {
                try
                {
                    using (SqlCommand cmd = new SqlCommand("DELETE FROM Recete_tbl WHERE İlacID=@ID", con))
                    {
                        cmd.Parameters.AddWithValue("@ID", ID);
                        con.Open();
                        cmd.ExecuteNonQuery();
        ...
        private void dataGridView1_RowHeaderMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            try
            {
                // ID'nin dönüşümünde hata oluşabilecek durumları ele alma
                ...
                txtkullanimi.Text = dataGridView1.Rows[e.RowIndex].Cells[2].Value?.ToString() ?? string.Empty;
                rtxtilacdetay.Text = dataGridView1.Rows[e.RowIndex].Cells[3].Value?.ToString() ?? string.Empty;
            }
